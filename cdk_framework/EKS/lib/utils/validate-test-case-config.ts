...
const configKeys = new Set([
  'cluster_name',
  'sample_app_image_uri',
  'sample_app_mode',
  'collector_config'
]);
export function validateTestcaseConfig(
  info: any,
  clusterStackMap: Map<string, FargateStack | EC2Stack>
) {
  ...
  switch (key) {
    case 'cluster_name':
      if (clusterStackMap.get(value) == undefined) {
        throw Error(
          `Cluster name ${value} does not reference an existing cluster`
        );
      }
      break;
    case 'sample_app_mode':
      if (value !== 'push' && value !== 'pull') {
        throw new Error(
          `sampleAppMode must have value "push" or "push", "${value}" is invalid`
        );
      }
      break;
  }
