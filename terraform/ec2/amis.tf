...
variable "ami_family" {
  default = {
    ...
    linux = {
      ...
      install_command          = "for i in $(seq 1 30); do sudo fuser /usr/lib/sysimage/rpm/.rpm.lock /var/lib/rpm/.rpm.lock 2>/dev/null || break; echo 'Waiting for RPM lock...' && sleep 2; done; sudo rpm -Uvh aws-otel-collector.rpm"
      start_command            = "ADOT_CONFIG_URI=$(echo -n 'CONFIGURATION_URI_PLACEHOLDER' | base64 -d)\nsudo /opt/aws/aws-otel-collector/bin/aws-otel-collector-ctl -c \"$ADOT_CONFIG_URI\" -f FEATUREGATE_PLACEHOLDER -a start"
      status_command           = "sudo /opt/aws/aws-otel-collector/bin/aws-otel-collector-ctl -a status"
      ssm_validate             = "sudo /opt/aws/aws-otel-collector/bin/aws-otel-collector-ctl -a status | grep running"
      connection_type          = "ssh"
    ...
    windows = {
      ...
      install_command          = "Start-Process msiexec -ArgumentList '/i','C:\\aws-otel-collector.msi','/quiet','/norestart' -Wait -PassThru | ForEach-Object { if ($_.ExitCode -ne 0) { throw \"MSI install failed with exit code $($_.ExitCode)\" } }"
