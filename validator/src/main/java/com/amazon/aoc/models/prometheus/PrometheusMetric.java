...
public class PrometheusMetric {
    ...
    /**
     * Check if a metric should be skipped (when validating). ADOT adds certain
     * metrics when converting Prometheus metrics to the OTLP dataformat (such as
     * the +Inf bound histogram).
     *
    ...
    /**
     * Comparison method for comparing two Prometheus metrics by the metric name and
     * labels.
     */
    ...
    /**
     * Strict comparison method for comparing two Prometheus metrics by the metric
    ...
    private static List<String> labelsMapToList(Map<String, String> labelsMap) {
        ...
        for (Map.Entry<String, String> entry : labelsMap.entrySet()) {
            ...
            // by 'key'.
            if (!key.equals(HISTOGRAM_BOUND_FIELD) && !key.equals(QUANTILE_FIELD) && key.indexOf(KEY_FIELD) != 0) {
                continue;
            }
