...
public class App implements Callable<Integer> {
	...
	@CommandLine.Option(names = {"-t", "--testing-id"}, defaultValue = "dummy-id")
	...
	@CommandLine.Option(names = {"--metric-namespace"}, defaultValue = "AWSObservability/CloudWatchOTService")
	private String metricNamespace;

	@CommandLine.Option(names = {"--endpoint"})
	private String endpoint;

	@CommandLine.Option(names = {"--region"}, defaultValue = "us-west-2")
	...
	@CommandLine.Option(names = {"--alarm-names"}, description = "the cloudwatch alarm names")
	private List<String> alarmNameList;

	@CommandLine.Option(names = {"--mocked-server-validating-url"}, description = "mocked server validating url")
	private String mockedServerValidatingUrl;

	@CommandLine.Option(names = {"--canary"}, defaultValue = "false")
	private boolean isCanary;

	@CommandLine.Option(names = {"--testcase"}, defaultValue = "otlp_mock")
	private String testcase;

	@CommandLine.Option(names = {"--cortex-instance-endpoint"}, description = "cortex instance validating url")
	private String cortexInstanceEndpoint;

	@CommandLine.Option(names = {"--rollup"}, defaultValue = "true")
	...
	public Integer call() throws Exception {
		...
		// load config
		List<ValidationConfig> validationConfigList = new ConfigLoadHelper().loadConfigFromFile(configPath);
	...
	private void validate(Context context, List<ValidationConfig> validationConfigList) throws Exception {
	...
	private <T> T buildJsonContext(String metricContext, Class<T> clazz) throws JsonProcessingException {
