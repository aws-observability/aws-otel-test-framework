...
public class RetryHelper {
     ...
     *            sleep time among retries
     * @param retryable
     *            the lambda
     * @return false if retryCount exhausted
     * @throws Exception
     *             when the retry count is reached
     */
    public static boolean retry(int retryCount, int sleepInMilliSeconds, boolean throwExceptionInTheEnd,
            Retryable retryable) throws Exception {
    ...
    public static void retry(Retryable retryable) throws Exception {
        retry(Integer.valueOf(GenericConstants.MAX_RETRIES.getVal()),
                Integer.valueOf(GenericConstants.SLEEP_IN_MILLISECONDS.getVal()), true, retryable);
    }
     ...
     *            the total retry count
     * @param retryable
     *            the lambda function to be executed
     * @throws Exception
     *             when the retry count is reached
     */
    public static void retry(int retryCount, Retryable retryable) throws Exception {
        retry(retryCount, Integer.valueOf(GenericConstants.SLEEP_IN_MILLISECONDS.getVal()), true, retryable);
    }
}
