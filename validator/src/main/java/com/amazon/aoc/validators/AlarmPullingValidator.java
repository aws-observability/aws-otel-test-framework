...
public class AlarmPullingValidator implements IValidator {
    ...
    public void validate() throws Exception {
        ...
        Dimension dimension = new Dimension().withName(TEST_CASE_DIM_KEY).withValue(context.getTestcase());
        RetryHelper.retry(this.pullTimes, this.pullDuration * 1000, false, () -> {
            List<MetricAlarm> alarmList = this.cloudWatchAlarmService.listAlarms(context.getAlarmNameList());
            ...
            for (MetricAlarm metricAlarm : alarmList) {
                ...
                if (!metricAlarm.getStateValue().equals("OK")) {
                    log.error(
                            "alarm {} is alarming, status is {}, " + "metric is {}, " + "matric error : {}, "
                                    + "failing to bake",
                            metricAlarm.getAlarmName(), metricAlarm.getStateValue(), metricAlarm.getMetrics(),
