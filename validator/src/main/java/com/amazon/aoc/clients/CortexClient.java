...
public class CortexClient {
	...
	public CortexClient(Context context) {
		...
		this.httpClient = HttpClients.custom()
				.addInterceptorLast(new AWSRequestSigningApacheInterceptor(APS_SERVICE_NAME, signer,
						new DefaultAWSCredentialsProviderChain()))
				.setRetryHandler(new DefaultHttpRequestRetryHandler())
				.setDefaultRequestConfig(RequestConfig.custom().setConnectionRequestTimeout(2000).build()).build();
	...
	public List<PrometheusMetric> query(String query, String timestamp)
		...
		URI uri = new URIBuilder().setScheme(Objects.requireNonNull(rawUrl).scheme()).setHost(rawUrl.host())
				.setPort(rawUrl.port()).setPath(rawUrl.encodedPath() + "/api/v1/query").addParameter("query", query)
				.addParameter("time", timestamp).build();
	...
	/**
	 * list metrics for the last one hour interval from the cortex instance via the
	 * query endpoint.
	 *
	 * @param query
	 *            the Prometheus expression query string
	...
	public List<PrometheusMetric> queryMetricLastHour(String query)
		...
		URI uri = new URIBuilder().setScheme(Objects.requireNonNull(rawUrl).scheme()).setHost(rawUrl.host())
				.setPath(rawUrl.encodedPath() + "/api/v1/query").addParameter("query", query)
				.addParameter("start", Long.toString(endEpochTime - 3600))
				.addParameter("end", Long.toString(endEpochTime)).addParameter("step", "15").build();
