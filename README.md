...
* Contributors won't need to build the validation logic for their own backends. We use our own version of [mocked backend](mocked_server/) in the integration and soaking test. Inside this mock server, we only validate if the data is received but not the data accuracy. [The mechanism of testing framework](docs/mechanism-of-testing-framework.md)
