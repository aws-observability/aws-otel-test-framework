	...
	for _, test := range tests.Tests {
		...
		for _, testPlatform := range test.Platforms {
			...
			if _, ok := runConfig.IncludedServices[testPlatform]; ok {
				...
				case "EKS_FARGATE", "EKS_ADOT_OPERATOR", "EKS_ADOT_OPERATOR_ARM64", "EKS", "EKS_ARM64", "EKS_JAVA_INSTRUMENTATION_ADOT_OPERATOR", "EKS_JAVA_INSTRUMENTATION_ADOT_OPERATOR_ARM64":
